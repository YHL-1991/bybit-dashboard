"""
청산 히트맵 *추정* 로직 v2  (⚠️ 실제 청산 데이터 아님)

주의: 이 모듈은 레버리지 분포 가정 + 가우시안으로 만든 *추정/장식*값이며
실제 거래소 청산 데이터가 아니다. 또한 프로덕션(Railway)에서는 CloudFront가
서버측 Bybit 호출을 차단하므로 이 경로는 사실상 dead code다. 프론트엔드가
실시간 호가창 매물벽을 직접 받아 추정하는 쪽(static/app.js)이 실사용 경로다.
